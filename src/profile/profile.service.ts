...
export class ProfileService {
  ...
  public async getSelfProfile(user_id: number): Promise<SelfProfileResponseDto> {
  ...
  public async confirmEmail(token: string): Promise<SuccessResponseDto> {
    ...
    await this.userService.update(decoded_activation_token.user_id, { email_confirmed: true });

    return new SuccessResponseDto('Почта подтверждена');
  }

  public async changePassword(user_id: number, dto: ChangePasswordDto): Promise<SuccessResponseDto> {
    const user = await this.userService.findByUserId(user_id);
    const isValidPassword = await this.userService.isValidPassword(dto.old_password, user.password);
    if (!isValidPassword) {
